...
import { streamGenerate, processTemplate, callApiSource, callMcpTool } from "../lib/api"
...
export function ComposeTab() {
  ...
  const handleModifierAction = async (action: (typeof config.quickActions)[0]) => {
    ...
    if (action.source.type === "ollama") {
      ...
      const resolved = resolveRoleProvider("chat", config.ai.providers, config.ai.assignments, {
        fallbackModel: config.ai.selectedModel,
      })
      if (!resolved.model || !config.backend.isInstalled) {
      ...
      try {
        await streamGenerate(
          config.backend.url,
          { prompt, model: resolved.model, byok: resolved.byok },
          (chunk) => {
  ...
  const handleSave = () => {
    ...
    if (isEditing) {
      updateMutation.mutate({ id: editingPrompt!.id, data: { title, content, category, tags } }, { onSuccess, onError })
    } else {
  ...
  const isSaveDisabled =
    !editorText.trim() || !config.backend.isInstalled || createMutation.isPending || updateMutation.isPending
    ...
    <div className="flex flex-col h-full">
      ...
      {modifierError && (
        <div className="mx-4 mt-3 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg flex items-start gap-2">
          ...
          <button
            onClick={() => setModifierError(null)}
            className="ml-auto text-amber-500/60 hover:text-amber-400 shrink-0"
          >
            <X size={12} />
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 pb-20">
        ...
        {/* Editor */}
        <div>
          <label className="block text-xs font-medium text-slate-400 mb-1.5">Prompt Editor</label>
          <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 transition-all">
            ...
            <div className="flex flex-wrap items-center gap-0.5 p-1.5 border-b border-slate-800 bg-slate-950/50">
              ...
              <div className="flex items-center gap-0.5 pr-1.5 border-r border-slate-800 mr-1.5">
                ...
                <button
                  onClick={() => handleWrapSelection("<context>\n", "\n</context>")}
                    ...
                    {showSuggestions && (
                      ...
                      >
                        <div className="px-3 py-2 border-b border-slate-700 bg-slate-800/80">
                          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Modifiers</p>
                        </div>
        ...
        {detectedVars.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Template Variables</h3>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Live Preview</h3>
            </div>
            <button
              onClick={handleCopy}
              disabled={!editorText}
          ...
          <div className="bg-slate-950 border-l-2 border-indigo-500 border-r border-t border-b border-r-slate-800 border-t-slate-800 border-b-slate-800 rounded-lg p-3 min-h-[80px]">
            {editorText ? (
              ...
              <span className="text-xs text-slate-600 italic">Start typing to see preview...</span>
            )}
          </div>
        </div>
