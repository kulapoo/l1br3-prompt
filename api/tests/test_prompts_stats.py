...
def test_stats_aggregates_totals_and_favorites(client, db):
    ...
    _set_usage(db, a["id"], 3, datetime.now(UTC).isoformat())
    _set_usage(db, b["id"], 7, datetime.now(UTC).isoformat())
...
def test_stats_top_used_ordered_and_limited(client, db):
    ...
    for i in range(7):
        ...
        _set_usage(db, p["id"], i + 1, datetime.now(UTC).isoformat())
        ids.append((p["id"], i + 1))
...
def test_stats_excludes_soft_deleted(client, db):
    ...
    _set_usage(db, a["id"], 4, datetime.now(UTC).isoformat())
    _set_usage(db, b["id"], 10, datetime.now(UTC).isoformat())
