...
In PRD artifact mode, create `docs/plans/` if needed. If the PRD contains a `Delivery Milestones` table, update only the selected row from `pending` to `in-progress` and set its `Plan` cell to the generated plan path.
...
> **Need requirements first?** Use `/plan-prd` for a lean PRD at `docs/prds/{name}.prd.md`.
